...
app.get('/health', (_req, res) => res.json({ ok: true, service: 'sentinel-ai-server', pairing: Boolean(JWT_SECRET && CONTROL_PANEL_SECRET) }));
...
function requireControlSecret(req, res, next) {
  ...
  const a = Buffer.from(supplied); const b = Buffer.from(CONTROL_PANEL_SECRET);
  if (!a.length || a.length !== b.length || !crypto.timingSafeEqual(a, b)) return res.status(401).json({ error: 'invalid_control_credentials' });
  next();
}
...
app.get('/pairing/status/:sessionId', requireControlSecret, (req, res) => {
  cleanExpiredPairingSessions();
...
app.post('/pairing/claim', (req, res) => {
  ...
  if (!/^\d{6}$/.test(String(code || '')) || !/^[A-Za-z0-9._:-]{8,128}$/.test(String(deviceId || ''))) return res.status(400).json({ error: 'invalid_pairing_request' });
  const entry = [...pairingSessions.entries()].find(([, session]) => session.code === String(code));
  if (!entry) return res.status(410).json({ error: 'pairing_code_expired_or_used' });
  const [sessionId, session] = entry;
  if (session.claimed || session.expiresAt <= Date.now()) return res.status(410).json({ error: 'pairing_code_expired_or_used' });
...
app.post('/channel/ack', auth, (req, res) => {
  ...
  if (typeof commandId !== 'string' || commandId.length < 8 || commandId.length > 128) return res.status(400).json({ error: 'invalid_command_id' });
...
app.post('/commands', requireControlSecret, (req, res) => {
  ...
  const queue = commandQueues.get(deviceId) || [];
  queue.push(command); commandQueues.set(deviceId, queue);
