...
export default function App() {
  ...
  const [token, setToken] = useState(localStorage.getItem('sentinelToken') || '');
  const [controlSecret, setControlSecret] = useState(sessionStorage.getItem('sentinelControlSecret') || '');
  ...
  function saveControlSecret(value: string) {
    setControlSecret(value);
    if (value) sessionStorage.setItem('sentinelControlSecret', value);
    else sessionStorage.removeItem('sentinelControlSecret');
  }

  function log(text: string, status: string) {
    setLogs(p => [{ time: new Date().toLocaleTimeString(), text, status }, ...p].slice(0, 10));
  }
  ...
  async function startPairing() {
    ...
    try {
      const r = await fetch(`${API}/pairing/start`, { method: 'POST', headers: { 'x-control-secret': controlSecret.trim() } });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'pairing_start_failed');
      setPairingCode(d.code); setPairingSession(d.sessionId); setPairingOpen(true);
      log('Pairing session started', `Code ${d.code}`);
    } catch (e) { log('Pairing session', e instanceof Error ? e.message : 'Network error'); }
    finally { setPairingBusy(false); }
  }

  useEffect(() => {
    if (!pairingSession || !controlSecret) return;
    let stopped = false;
    const poll = async () => {
      try {
        const r = await fetch(`${API}/pairing/status/${encodeURIComponent(pairingSession)}`, { headers: { 'x-control-secret': controlSecret } });
        const d = await r.json();
        if (!r.ok) throw new Error(d.error || 'pairing_status_failed');
        if (d.status === 'paired' && d.deviceId && d.token && !stopped) {
          localStorage.setItem('sentinelDeviceId', d.deviceId); localStorage.setItem('sentinelToken', d.token);
          setDeviceId(d.deviceId); setToken(d.token); setConnected(true); setPairingOpen(false); setPairingSession('');
          log('Android companion paired', 'Authenticated channel ready');
        }
      } catch (e) { if (!stopped) log('Pairing channel', e instanceof Error ? e.message : 'Network error'); }
    };
  ...
  async function sync() {
    if (!token || !deviceId || !API) return setConnected(false);
    try {
      const r = await fetch(`${API}/devices/${encodeURIComponent(deviceId)}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) return setConnected(false);
      const d = await r.json(); setConnected(Boolean(d.connected));
    } catch { setConnected(false); }
  }

  useEffect(() => { sync(); const id = window.setInterval(sync, 5000); return () => window.clearInterval(id); }, [token, deviceId]);

  async function run() {
    const text = command.trim();
    if (!text || running) return;
    if (!controlSecret || !deviceId) return log(text, 'Pair Android first');
    setRunning(true);
    try {
      const r = await fetch(`${API}/commands`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-control-secret': controlSecret }, body: JSON.stringify({ deviceId, type: 'open_url', payload: { url: text } }) });
      const d = await r.json(); log(text, r.ok ? `Accepted · ${d.command?.commandId?.slice(0, 8) || 'queued'}` : (d.error || 'Rejected'));
    } catch { log(text, 'Network error'); }
  ...
  return <div className="agent-shell">
    <aside className="sidebar">
      ...
      <nav><a className="active" href="#dashboard"><Activity size={17}/> Dashboard</a><a href="#commands"><Command size={17}/> Commands</a><a href="#permissions"><Lock size={17}/> Permissions</a><a href="#android"><Smartphone size={17}/> Android</a></nav>
      <div className="device"><small>ANDROID COMPANION</small><b><span className={connected?'dot on':'dot'}/> {connected?'Connected':'Not connected'}</b><p>{connected?'Authenticated device channel is active.':'Pair the companion app before device actions can execute.'}</p><button className="secondary" onClick={() => setPairingOpen(true)}>{connected?'Manage pairing':'Pair device'}</button></div>
    </aside>
    <main id="dashboard">
