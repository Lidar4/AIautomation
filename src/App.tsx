...
export default function App() {
  ...
  const [deviceId, setDeviceId] = useState(localStorage.getItem('sentinelDeviceId') || ''); const [token, setToken] = useState(localStorage.getItem('sentinelToken') || '');
  ...
  useEffect(() => {
    ...
    const poll = async () => { try { const r = await fetch(`${API}/pairing/status/${encodeURIComponent(pairingSession)}`, { headers: { 'x-control-secret': controlSecret } }); const d = await r.json(); if (!r.ok) throw new Error(d.error || 'pairing_status_failed'); if (d.status === 'paired' && d.deviceId && d.token && !stopped) { localStorage.setItem('sentinelDeviceId', d.deviceId); localStorage.setItem('sentinelToken', d.token); setDeviceId(d.deviceId); setToken(d.token); setConnected(true); setPairingOpen(false); setPairingSession(''); log('Android companion paired', 'Authenticated channel ready'); } } catch (e) { if (!stopped) log('Pairing channel', e instanceof Error ? e.message : 'Network error'); } };
    poll(); const id = window.setInterval(poll, 2000); return () => { stopped = true; window.clearInterval(id); };
  }, [pairingSession, controlSecret]);

  async function sync() { if (!token || !deviceId || !API) return setConnected(false); try { const r = await fetch(`${API}/devices/${encodeURIComponent(deviceId)}`, { headers: { Authorization: `Bearer ${token}` } }); if (!r.ok) return setConnected(false); const d = await r.json(); setConnected(Boolean(d.connected)); } catch { setConnected(false); } }
